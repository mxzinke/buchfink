...
// ExtractFromPDF returns the structured invoice data embedded in a hybrid PDF.
//
...
func ExtractFromPDF(pdf []byte) (*EmbeddedFile, error) {
...
// ParseAny reads an invoice from whatever arrived: a bare XML file in either
// syntax, or a hybrid PDF with the data embedded.
//
// This is what a receiving path actually needs. What lands in the inbox is a
// file, and whether the structured part sits in the file itself or inside a PDF
// is not the recipient's problem — it is a question about the sender's software.
func ParseAny(data []byte) (*Invoice, error) {
	if !IsPDF(data) {
		return Parse(data)
	}
	embedded, err := ExtractFromPDF(data)
	if err != nil {
		return nil, err
	}
	inv, err := Parse(embedded.Data)
	if err != nil {
		return nil, fmt.Errorf("der eingebettete Datensatz %s konnte nicht gelesen werden: %w",
			embedded.Name, err)
	}
	return inv, nil
}
