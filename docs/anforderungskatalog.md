...
| Fehlende Pflichtangaben blockieren die Freigabe und werden benannt | 🟡 | internal/domain/receipt.go:260-332 prüft streng je Datei; inhaltliche Pflichtangaben werden mangels Feldern nicht geprüft | 6 |
| Belegnummernkreis je Geschäftsjahr und Belegart konfigurierbar, ohne Doppelvergabe | 🟡 | internal/domain/numberrange.go:21-26 trennt je Jahr und Art, internal/repository/numberrange_gorm.go:22-30 vergibt ohne Doppelvergabe; die Systematik ist seit Welle 5b nur für den Rechnungskreis einstellbar (internal/domain/numberrange.go:77-104, internal/domain/settings.go:40-42), der Belegkreis schreibt weiterhin fest ER-JJJJ-NNNN (internal/domain/numberrange.go:61) | 6 |

**Stand.** Die Nummernvergabe ist vorbildlich, die Belegkopfdaten fehlen: ein abgelegter, noch nicht gebuchter Beleg trägt weder Datum noch Aussteller noch Betrag (internal/domain/receipt.go:162-213). Welle 5b hat den Nummernkreis der Ausgangsrechnung einstellbar gemacht; der Belegkreis behält sein festes Format. Welle 6.
...
| Offene-Posten-Liste je Debitor und Kreditor zu jedem Stichtag, mit Fälligkeit und Altersstruktur | 🟡 | internal/service/payment_service.go:185-198 (`OpenItemsAt`) liest die zum Stichtag offenen Buchungen und die bis dahin gebuchten Ausgleiche, beide Grenzen auf dem Buchungsdatum; die Oberfläche führt den Stichtag als Feld und rechnet die Überfälligkeit gegen ihn (frontend/src/pages/AccountsPage.tsx:159-169, :393, :622-630). Die ausgestellte Abschlagsrechnung ist seit Welle 5b eine zweite Quelle derselben Liste, weil sie einen offenen Posten ohne Buchung trägt (internal/service/advance_service.go:602-637, frontend/src/pages/BankImportPage.tsx:109). Eine Altersstruktur gibt es weiterhin nicht | 6 |
| Restlaufzeiten für §§ 268 Abs. 4, 5 und 285 Nr. 1 HGB auswertbar | ❌ | internal/domain/payment.go:110-141; `OpenItem` kennt kein Restlaufzeitband, keine Auswertung gliedert danach | 6 |
...
| Storno und Neubuchung mit der Ursprungsbuchung verknüpft, in beide Richtungen navigierbar | 🟡 | internal/domain/journal.go:154 verknüpft Storno und Ursprung über `ReversalOfID`; bei einer Rechnung führt Welle 5b die Kette in beide Richtungen — die Berichtigung nennt die berichtigte Rechnung, die stornierte ihr Stornodokument (internal/domain/invoice.go:306-312, internal/service/invoice_correction.go:170-171, frontend/src/pages/InvoicesPage.tsx:438). Die Neubuchung nach dem Storno einer Handbuchung hängt weiterhin an nichts, es gibt kein Feld dafür | 6 |
...
**Stand.** Storno und Korrektur sind streng gebaut. Auf der Rechnungsseite schließt Welle 5b die Verkettung: Stornodokument und Ursprungsrechnung zeigen aufeinander, und die berichtigte Rechnung trägt Nummer und Datum der Rechnung, an deren Stelle sie tritt. Im Journal endet die Kette weiterhin nach dem Storno — welche Handbuchung die stornierte ersetzt, steht nirgends. Welle 6.
...
| Nummernkreise je Mandant, Geschäftsjahr, Belegart oder Standort konfigurierbar, Systematik dokumentiert | 🟡 | internal/domain/numberrange.go:21-26 trennt je Mandant, Jahr und Art; die Systematik des Rechnungskreises ist seit Welle 5b einstellbar (:77-104 mit den Platzhaltern für Jahr und Zähler, internal/domain/settings.go:40-42, frontend/src/pages/SettingsPage.tsx:415), und ein Format ohne Zähler wird abgewiesen statt stillschweigend ergänzt (internal/domain/numberrange.go:108-121). Eine Verfahrensdokumentation, in der die gewählte Systematik steht, gibt es im Produkt weiterhin nicht (PRF-03) | 6 |
...
| B. Beleg, Journal, Konten | BEL-01 bis BEL-09 | 18 | 11 | 2 | 4 | 35 |
...
| E. Ausgangsrechnungen und E-Rechnung | RECH-01 bis RECH-10 | 29 | 6 | 2 | 6 | 43 |
