...
export const InvoicesPage: React.FC = () => {
  ...
  const [contactName, setContactName] = useState('Acme Corp GmbH');
  ...
  const handleAddItem = () => {
    ...
    setItems([
      ...
      {
        position: nextPos,
        description: 'Neue Leistung / Artikel',
        quantity: 1,
        unit: 'Stück',
        unitPrice: 100.0,
        taxRate: 0.19,
  ...
  const handleCreateInvoice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessToast(`Rechnung ${invNumber} erfolgreich erstellt und ZUGFeRD-konform archiviert.`);
      await loadInvoices();
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-stone-50 border-b border-stone-200 text-stone-500 font-medium">
              <tr>
                ...
                <th className="py-3 px-4">Empfänger (Kunde)</th>
                <th className="py-3 px-4 text-right">Netto</th>
                <th className="py-3 px-4 text-right">USt</th>
                <th className="py-3 px-4 text-right">Gesamtbrutto</th>
            ...
            <tbody className="divide-y divide-stone-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="py-8 text-center text-stone-400">
                    Noch keine Ausgangsrechnungen erstellt. Klicken Sie auf "Neue Ausgangsrechnung erstellen".
                  </td>
                </tr>
              ) : (
                invoices.map((inv) => (
        ...
        <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full p-6 border border-stone-200 animate-in fade-in my-8 max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleCreateInvoice} className="flex-1 overflow-y-auto py-4 space-y-5 text-xs">
              ...
              {/* Summary Totals */}
              <div className="p-4 bg-stone-100 rounded-xl border border-stone-200 space-y-1 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-xs font-semibold bg-amber-600 hover:bg-amber-700 text-white rounded-lg shadow-xs flex items-center gap-2"
                >
                  <CheckCircle2 className="w-4 h-4" />
                  {isSubmitting ? 'Wird gespeichert...' : 'Rechnung anlegen & ZUGFeRD archivieren'}
                </button>
              </div>
            </form>
          </div>
