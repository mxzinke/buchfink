...
const out = resolve('.cache/bughunt-2026-09-12/evidence/followup/year-end');
...
const tenant = await run('CreateTenant', 'Nordlicht Nachprüfung UG (haftungsbeschränkt)',
  '/tmp/buchfink-bughunt-followup-2026-09-12/year-2025', {
    ...settings, companyName: 'Nordlicht Nachprüfung UG (haftungsbeschränkt)',
    fiscalYear: 2025, contactName: 'Nora Beispiel',
    invoiceCheckSince: '2025-01-01', inGruendung: true,
  });
await call('SwitchTenant', tenant.id);
// Setup must already have selected and created 2025.
if (await call('GetFiscalYear') !== 2025) throw Error('Selected fiscal year was lost');
...
An Nordlicht Nachprüfung UG (haftungsbeschränkt), Musterweg 12, 10405 Berlin
